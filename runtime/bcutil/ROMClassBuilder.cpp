...
ROMClassBuilder::handleAnonClassName(J9CfrClassFile *classfile, bool *isLambda)
{
    ...
    for(i = originalStringLength - 2; i >= 8; i--){
        if('$' == *(originalStringBytes + i - 8)
			...
			&& 'L' == *(originalStringBytes + i - 6)
			&& 'a' == *(originalStringBytes + i - 5)
			&& 'm' == *(originalStringBytes + i - 4)
			&& 'b' == *(originalStringBytes + i - 3)
			&& 'd' == *(originalStringBytes + i - 2)
			&& 'a' == *(originalStringBytes + i - 1)
			&& '$' == *(originalStringBytes + i)
        	) {
            *isLambda = true;
            break;
        }
...
ROMClassBuilder::prepareAndLaydown( BufferManager *bufferManager, ClassFileParser *classFileParser, ROMClassCreationContext *context )
{
    ...
    /* to recover the class name for anonymous classes if Shared Classes is disabled, unavailable, or fails *
    U_8 savedNumber[124];
    U_32 savedNumberLength = 0;*/
    bool isLambda = false;
    
    if (context->isClassAnon()) {
        BuildResult res = handleAnonClassName(classFileParser->getParsedClassFile(), &isLambda);//, savedNumber, &savedNumberLength);
        if (OK != res) {
            return res;
        }
    }
